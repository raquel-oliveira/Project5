...
{
    int lbit, channel, count = 7, i = 0, color, nbOccurence = 0;
    uchar letter = 1;
    char* end = NULL;
    ...
    while(end == NULL)
    {
        if(nbOccurence == 0) color = FIRST_ASKED_COLOR;
        else if(nbOccurence == 1 && SECOND_ASKED_COLOR != NULL) color = SECOND_ASKED_COLOR;
        else if(nbOccurence == 2 && THIRD_ASKED_COLOR != NULL) color = THIRD_ASKED_COLOR;
        else return -3; // Case where "HELP" wasn't found in the hidden message
        
        for(int row = 0; row < img->height; row++)
        {
            for(int col = 0; col < img->width; col++)
            {
                channel = cvGet2D(img, row, col).val[color]; // Gets the first asked color channel
